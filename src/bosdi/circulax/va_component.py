...
Intended for use by the :mod:`bosdi.va` compiler, which lowers OpenVAF
...
def _install_custom_jvp(
    ...
    *,
    ...
    _jvp_dynamic_names: tuple[str, ...] = getattr(
        cls, "_diff_param_names", _param_names_for_kw
    )
    ...
    def _unpack(vars_vec: jax.Array, params: Any) -> tuple[Any, Any, dict]:
        ...
        if _has_init_arg:
            if hasattr(params, "_init_cache_v2"):
                kw["init"] = params._init_cache_v2
            else:
                # @<Name>.setup registers after class construction; read it
                # dynamically so the JVP path sees the registered setup fn.
                # When the wrapper uses **kwargs (emitted _register_setup),
                # pass all params through so the inner setup gets real values.
                _setup_ref = getattr(cls, "_setup_fn_ref", None)
                if _setup_ref is not None:
                    _setup_sig = inspect.signature(_setup_ref)
                    _has_var_kw = any(
                        p.kind == inspect.Parameter.VAR_KEYWORD
                        for p in _setup_sig.parameters.values()
                    )
                    if _has_var_kw:
                        kw["init"] = _setup_ref(**kw)
                    else:
                        _accepts = {p.name for p in _setup_sig.parameters.values()}
                        kw["init"] = _setup_ref(
                            **{k: v for k, v in kw.items() if k in _accepts}
                        )
                else:
                    kw["init"] = {}
        elif _has_cache:
            if _jvp_is_specialized and _jvp_dynamic_names:
                ...
                kw["_init_cache"] = _jvp_setup_fn(
                    **{n: kw[n] for n in _param_names_for_kw}
                )
    ...
    raw_physics = (
        cls._fast_physics
    )  # the unwrapped static closure built by _build_component
    ...
    def fast_physics_jvp(
        ...
        def _mat(leaf: Any) -> Any:
            ...
            return (
                jnp.zeros(leaf.aval.shape, leaf.aval.dtype) if _is_zero(leaf) else leaf
            )
        ...
        if has_param_grad:
            ...
            _, (f_p_dot, q_p_dot) = jax.jvp(
                _params_time_fn, (params, t), (params_dot_m, t_dot_m)
            )
        else:
    ...
    if _has_combined:
        cls._combined_fn = staticmethod(_fast_combined)  # type: ignore[method-assign]
