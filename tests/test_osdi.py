...
def resistor_model():
    ...
    assert model.num_params == 2, f"Expected 2 parameters (R, m), got {model.num_params}"
...
def test_resistor_dc_evaluation(resistor_model):
    ...
    np.testing.assert_allclose(cur, expected_cur, rtol=1e-6, 
                               err_msg="Currents do not match Ohm's Law")
    ...
    np.testing.assert_allclose(cond, expected_cond, rtol=1e-6, 
    ...
    np.testing.assert_allclose(chg, np.zeros_like(chg), atol=1e-12, 
                               err_msg="Static resistor should not accumulate charge")
    np.testing.assert_allclose(cap, np.zeros_like(cap), atol=1e-12, 
...
def test_resistor_batched_evaluation(resistor_model):
    ...
    params = jax.random.uniform(subkey, shape=(num_devices, 2), minval=10.0, maxval=1000.0)
    ...
    np.testing.assert_allclose(cur[42, 0], expected_i_a, rtol=1e-6,
